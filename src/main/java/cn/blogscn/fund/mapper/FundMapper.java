...
public interface FundMapper extends BaseMapper<Fund> {
    ...
    @Select("select avg(dwjz) from (select dwjz from fund where fsrq < #{fsrq} and fund_code = #{fund_code} order by fsrq desc limit 5) x")
    BigDecimal avgWeek(@Param("fsrq")String fsrq,@Param("fund_code") String fundCode);

    @Select("select avg(dwjz) from (select dwjz from fund where fsrq < #{fsrq} and fund_code = #{fund_code} order by fsrq desc limit 21) x")
    BigDecimal avgMonth(@Param("fsrq")String fsrq,@Param("fund_code") String fundCode);

    @Select("select avg(dwjz) from (select dwjz from fund where fsrq < #{fsrq} and fund_code = #{fund_code} order by fsrq desc limit 63) x")
